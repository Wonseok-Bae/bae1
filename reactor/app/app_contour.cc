...
using namespace std;//ok
 ...
 std::cout<<"app1_ start to run"<<std::endl;//첫 번째 아웃풋ok
...
//=============================================================================================================2.1 필요한 변수 res, rep만들기========
 ...
 //std::cout<<"app2_ I've set some inputs"<<std::endl;//ok
 ...
 rep->addSK(false); //여기가 데이터 피팅 - 어떻게?
 rep->setNBins(34);//reb변수에서 뽑기ok
 rep->setTime(6);//reb변수에서 뽑기ok
 //std::cout<<"app3_ ended up with setting us basic stuff"<<std::endl;//ok
 rep->FillEv(rep->getPullList());//여기서 matrix sum이랑 chi2 나옴 - 어떻게 되었더라?
 //std::cout<<"app4_ I've set some more & more inputs"<<std::endl;//ok
 ...
 rep->ifEqualIso(true); // Isotope 지정 235U unification하려면 이걸 true로 바꿔야 하는 건가?ok
 ...
 //std::cout<<"app5_ Getting current spectra"<<std::endl; //스펙트럼 얻기ok
 ...
 //std::cout<<"app6_ Have got current spectra"<<std::endl; //얻은 스펙트럼ok
 ...
 std::cout<<"app6.5_ cfMatrix "<<cfMatrix<<std::endl;
 outputFile -> Close();//ok
 
 std::cout<<"app7_ first thing saved(un-oscillation) -> outputFigs.root, cfMatrix"<<std::endl;
 ...
 double iST = atof(argv[2]);//입력한 값을 사인세타제곱으로ok
	...
	rep->getParVar(2)->setConstant(true);//이걸 자세히 파해쳐봐야 한다ok
 	rep->getParVar(6)->setConstant(true);//ok
	rep->getParVar(7)->setConstant(true);//ok
	rep->getParVar(8)->setConstant(true);//ok
 	RooMinuit m(*fcn);//m이란 변수를 fcn를 따와서 만든다.ok ★
    //[#1] INFO:Minization -- RooFormulaVar::defaultErrorLevel(fit) WARNING: Formula contains neither RooNLLVar nor RooChi2Var server, using default level of 1.0
    ...
    */
    ...
    /*
    **********
    **   10 **MIGRAD    1.05e+04       1e-06
    **********
    + HALF OF RESULT ★ MIGRAD MINUIT
    */
    ...
    std::cout<<"app8_ fit status code is : "<< res->status()<<std::endl;//fiting status = res->status variable ok	
	//status = 0    : OK
	...
	std::cout<<"app9_ quiality code of covariance matrix is : "<< res->covQual()<<std::endl;//cov matrix ok
 ...
 outText<<iDM<<" "<<iST<<" "<<bestFit<<std::endl;//output filed content ok
 ...
 //std::cout<<"app10_ size of output prediction list "<<outPrediction.size()<<std::endl;//ok
 //std::cout<<"app11_ size of output data list "<<outData.size()<<std::endl;//ok
 std::cout<<"app12_ chi2: "<<bestFit <<std::endl;//ok
