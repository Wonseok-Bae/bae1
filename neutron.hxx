...
TH2F* test = new TH2F("hist_number_of_signal", "number of signal;Lever Arm [cm]; Time [ns]", 20, 0, 200, 25, 0, 25); //새로 추가한 히스토그램
// }}}
    ...
    for (int ievent = 0; ievent < nevents; ievent++)
    {
        ...
        if (abs(t_vtx[0]) < 50 && abs(t_vtx[1]) < 50 && abs(t_vtx[2]) < 50)
        {
            ...
            for (int ineutronHit = 0; ineutronHit < 100; ineutronHit++)
            {
                ...
                // TODO: Change this according to the geometry
                if (abs(t_neutronHitX[ineutronHit]) < 120 && abs(t_neutronHitY[ineutronHit]) < 120 && abs(t_neutronHitZ[ineutronHit]) < 100)
                {
                    ...
                    if (signalWindow > 0)
                    {
                        ...
                        // Raise signal flag
                        is_Sig = true;
                        if(hitPerCube.at(key).energyDeposit > 0.5)      //에너지 threshold  
                            test->Fill(hitPerCube.at(key).trackLength,hitPerCube.at(key).timeWindow);       //히스토그램 채우기
                    }
                }
            }
