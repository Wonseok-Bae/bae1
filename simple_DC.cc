...
//===================================================================================================================================================================7.4 prediction graph again====== 
  ...
  TH1D* temp11;
  std::cout<<ifEHist<<std::endl;
  if(ifEHist == true){
    TFile eshift(EHistFile);
    temp11 = (TH1D*)eshift.Get("Escalefraction");
    for(int iii = 0 ; iii< fpredNEOS->GetNbinsX(); iii++){
      fpredNEOS->SetBinContent( iii+1, fpredNEOS->GetBinContent(iii+1) * (1 + temp11->GetBinContent(iii+1) * ((RooAbsReal*)_pulls->at(10))->getVal()) );   
    }    
    eshift.Close();
  }
